 ...
 * Return: 0 on success, error code otherwise.
 */
static int adf_ctl_ioctl_sla_create_rl_v2(unsigned long arg, bool compat)
{
	int ret = 0;
	...
	if (ret == 0) {
		...
		ret = rl_v2_create_sla(accel_dev, &sla, compat);
		if (!ret) {
 ...
 * Return: 0 on success, error code otherwise.
 */
static int adf_ctl_ioctl_sla_update_rl_v2(unsigned long arg, bool compat)
{
	int ret = 0;
	struct adf_user_sla sla;
	struct adf_accel_dev *accel_dev = NULL;
	...
	ret = get_accel_check_support(&accel_dev, &sla.pci_addr);
	if (ret == 0) {
		ret = rl_v2_update_sla(accel_dev, &sla, compat);
		adf_dev_put(accel_dev);
	}
