			   ...
			   enum vqat_type type)
{
	...
	for (i = 0; i < number; i++) {
		...
		if (pos < 0)
			return -EINVAL;
		cap = adf_vqat_caps_put(vcap, pos);
	}

	vcap->blk->len = adf_vqat_caps_blk_size(vcap);
	vcap->blk->number = number;
	pr_debug("cap blk length is %u, blk tail is %u\n",
		 vcap->blk->len,
		 vcap->tail);
				    ...
				    struct adf_vqat_class *dclass)
{
	...
	adf_vqat_caps_clone_blk(vcap, class_vcap->blk);
