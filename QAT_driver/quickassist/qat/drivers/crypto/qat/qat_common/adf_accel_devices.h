...
#ifdef QAT_ESXI
	int (*telemetry_control)(struct adf_accel_dev *accel_dev,
				 struct adf_tl_rp_indices *rp_indexes,
				 bool enable);
	int (*telemetry_get)(struct adf_accel_dev *accel_dev,
...
#ifdef QAT_ESXI
#define ADF_TL_CNTR_NAME_SIZE 64
struct adf_tl_cntr_desc {
	char name[ADF_TL_CNTR_NAME_SIZE];
	u64 val;
};

struct adf_tl_desc {
	size_t num_desc;
	struct adf_tl_cntr_desc *desc;
};
