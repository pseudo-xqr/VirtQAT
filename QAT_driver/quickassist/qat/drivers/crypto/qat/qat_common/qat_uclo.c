	...
	for (i = 0; i < obj_handle->uimage_num; i++) {
		...
		if (image->numpages > 1) {
			pr_err("Only 1 page is allowed in a UOF for CPM2X; We found %d in %s\n", \
				image->numpages, qat_uclo_get_string(&obj_handle->str_table, image->img_name));
			return -EINVAL;
		}
	}
			     ...
			     struct icp_qat_uof_image *uof_image)
{
	...
	if (IS_QAT_GEN3_OR_GEN4(obj_handle->prod_type)) {
				...
				struct icp_qat_fw_auth_desc **desc)
{
	...
	if (size > (ICP_QAT_AE_IMG_OFFSET(device_id) + ICP_QAT_CSS_MAX_IMAGE_LEN)) {
		       ...
		       void *addr_ptr, int mem_size)
{
	...
	if (handle->fw_auth) {
		...
		if (handle->pci_dev->device == ADF_C3XXX_PCI_DEVICE_ID ||
		    handle->pci_dev->device == ADF_C3XXX_PCI_DEVICE_ID2) {
			pr_err("QAT: C3XXX doesn't support unsigned MMP\n");
			return -EINVAL;
		}
		qat_uclo_wr_sram_by_words(handle, 0,
					  addr_ptr, mem_size);
	}
	return status;
}
