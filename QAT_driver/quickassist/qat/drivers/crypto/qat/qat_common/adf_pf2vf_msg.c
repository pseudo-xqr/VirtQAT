...
void adf_vf2pf_req_hndl(struct adf_accel_vf_info *vf_info)
{
	...
	case ADF_VF2PF_MSGTYPE_GET_SMALL_BLOCK_REQ:
		...
		if (msg_data >> ADF_VF2PF_BLOCK_REQ_CRC_SHIFT) {
			res = adf_iov_block_get_crc(accel_dev, blk_msg_type,
						    blk_byte_num, &data,
						    vf_info->compat_ver);
			if (res)
			...
			res = adf_iov_block_get_byte(accel_dev, blk_msg_type,
						     blk_byte_num, &data,
						     vf_info->compat_ver);
			if (res)
				blk_resp_type = ADF_PF2VF_BLOCK_RESP_TYPE_ERROR;
...
{
	accel_dev->hw_device->disable_pf2vf_interrupt(accel_dev);
