...
static u32 translate_idle_delay(u32 idle_delay)
{
	if (idle_delay <= ADF_4XXX_PM_IDLE_8_US_VAL)
		return ADF_4XXX_PM_IDLE_8_US;
	else if (idle_delay <= ADF_4XXX_PM_IDLE_16_US_VAL)
		return ADF_4XXX_PM_IDLE_16_US;
	else if (idle_delay <= ADF_4XXX_PM_IDLE_32_US_VAL)
		return ADF_4XXX_PM_IDLE_32_US;
	else if (idle_delay <= ADF_4XXX_PM_IDLE_64_US_VAL)
		return ADF_4XXX_PM_IDLE_64_US;
	else if (idle_delay <= ADF_4XXX_PM_IDLE_128_US_VAL)
		return ADF_4XXX_PM_IDLE_128_US;
	else if (idle_delay <= ADF_4XXX_PM_IDLE_256_US_VAL)
		return ADF_4XXX_PM_IDLE_256_US;
	else
		return ADF_4XXX_PM_IDLE_512_US;
}
...
int adf_4xxx_init_pm(struct adf_accel_dev *accel_dev)
{
	...
	u32 val = 0;
	void __iomem *pmisc =
		(&GET_BARS(accel_dev)[ADF_4XXX_PMISC_BAR])->virt_addr;

	if (get_cfg_pm_setting(accel_dev,
			       &idle_delay_us,
			       ADF_PM_IDLE_INTERRUPT_DELAY))
		return -EFAULT;
	idle_delay = translate_idle_delay(idle_delay_us);
...
bool adf_send_pm_host_msg(struct adf_accel_dev *accel_dev)
{
	u32 msg = 0;
	int times = 0;
...
void adf_pm_bh_handler(struct work_struct *work)
{
	...
	if (pm_int_sts & ADF_4XXX_PM_IDLE_STS) {
		...
		if (!adf_send_pm_host_msg(accel_dev))
			dev_err(&GET_DEV(accel_dev),
...
bool adf_4xxx_pm_check_interrupts(struct adf_accel_dev *accel_dev)
{
	...
	INIT_WORK(&pm_irq->pm_irq_work, adf_pm_bh_handler);
